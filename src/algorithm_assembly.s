...
INPUT_GETTER
    ...
    LDR R11,[R3]
    TST R11,#0x1
    BNE INPUT_GETTER    ; USED AS A LABEL
    ...
    MOV R10,#0  ; TEMPORARY REGISTER FOR LOAD
...
LOOP
    ...
    LDR  R9,[R6,#-17]
    MOV  R8,R9			
    ADD  R7,R7,R8,LSL#1		; WEIGHT 2 FOR THE TOP ELEMENT
    MOV  R8,#0
    MOV  R9,#0
    
    LDR  R9,[R6,#-1]
    MOV  R8,R9      	
    ADD  R7,R7,R8,LSL#1		; WEIGHT 2 FOR THE LEFT ELEMENT
    MOV  R8,#0
    MOV  R9,#0
    
    LDR  R9,[R6,#+1]
    MOV  R8,R9		
    ADD  R7,R7,R8,LSL#1		; WEIGHT 2 FOR THE RIGHT ELEMENT
    MOV  R8,#0
    MOV  R9,#0
...
NEWLINE
    MOV  R1,#0  ; RESET THE COLUMN COUNTER
...
EOF	B	EOF     ; END OF THE ALGORITHM	
